...
public class AdminController extends HttpServlet {
    ...
    // ============================================================
    // FIXED: MOVE ORDER USING CUSTOMER_ID + SERVICES + CREATED_AT
    // ============================================================
    private void moveOrder(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            Order result = null;
            switch (action) {
                case "to_pending": result = service.moveToPending(customerId, services, timestamp); break;
                case "to_wash": result = service.moveToWash(customerId, services, timestamp); break;
                case "to_dry": result = service.moveToDry(customerId, services, timestamp); break;
                case "to_iron": result = service.moveToIron(customerId, services, timestamp); break;
                case "to_fold": result = service.moveToFold(customerId, services, timestamp); break;
                case "for_pickup": result = service.moveToForPickup(customerId, services, timestamp); break;
                case "to_deliver": result = service.moveToDeliver(customerId, services, timestamp); break;
                case "claimed": result = service.moveToClaimed(customerId, services, timestamp); break;
                case "claimed_from_pickup": result = service.moveToClaimedFromPickup(customerId, services, timestamp); break;
                default: result = null;
            }
            
            if (result != null) {
                out.write("{\"success\":true,\"order\":{\"id\":" + result.getId() + ",\"customerId\":" + result.getCustomerId() + ",\"status\":\"" + action + "\"}}");
            } else {
    ...
    private void updateSetPricingWeightAndPrice(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            boolean success = service.updateSetPricingWeightAndPrice(customerId, services, timestamp, weight, price);
            if (success) {
                out.write("{\"success\":true,\"order\":{\"customerId\":" + customerId + ",\"weight\":" + weight + ",\"price\":" + price + "}}");
            } else {
